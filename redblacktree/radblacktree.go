...
const (
	...
	nilByteArray [NumberOfBytes]byte = [NumberOfBytes]byte{}
)

// New instantiates a red-black tree with the built-in comparator for K
func NewRedBlackTree[K comparable](comparator Comparator[K]) *RedBlackTree[K] {
	return &RedBlackTree[K]{Comparator: comparator}
}

// Put inserts node into the tree.
// Key should adhere to the comparator's type assertion, otherwise method panics.
func (tree *RedBlackTree[K]) Put(key K) {
	var insertedNode *RedBlackTreeNode
	if tree.Root ==  {
